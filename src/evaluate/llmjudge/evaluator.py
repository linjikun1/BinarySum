...
class LLMJudgeEvaluator:
    ...
    def __init__(self, profile=None, api_key=None, base_url=None, model_name=None, temperature=0.1):
        """
        Initialize the LLM Judge Evaluator.
        
        Args:
            profile: Config profile name (e.g., "default", "aizex")
            api_key: OpenAI API key (fallback)
            base_url: API base URL (fallback)
            model_name: Model name (fallback)
            temperature: Temperature for generation
        """
        # Get config from profile or use provided params
        if profile:
